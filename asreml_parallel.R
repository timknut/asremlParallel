...
phenotype = "C04"
region <- "Chr1:14-2000"
phenofile <- "/mnt/users/tikn/Projects/Fatty_acids_bovine/GWAS/asreml/Data/AM_dyd_20_obs.txt"
...
run_asremlParallel <- function(n_jobs, phenotype, region, phenofile, pedigree){
# # Set region variables. -------------------------------------------------
if(length(unlist(strsplit(region, ":|-"))) != 3) {
  stop(sprintf("Region string: %s should be in format: 'Chr:from_bp-to_bp'", region))
  }
...
# Check filenames ---------------------------------------------------------
if(!file.exists(phenofile)) stop(sprintf("phenofile: %s does not exist", phenofile))
if(!file.exists(pedigree)) stop(sprintf("pedigreefile: %s does not exist", pedigree))
...
pheno <- data.table::fread(phenofile, verbose = F, showProgress = F)
...
# Check file paths --------------------------------------------------------
if(!file.exists(genofile)) stop(sprintf("genofile: %s does not exist", genofile))
if(!file.exists(mapfile)) stop(sprintf("mapfile: %s does not exist", pedigree))

variant_map <-
  data.table::fread(mapfile, verbose = FALSE, data.table = FALSE, showProgress = F)
variant_map <-
...
geno <- subset_common(genolist$genos, pheno = pheno) ## Change to just use common animals geno/pheno, and report Diff like gcta
...
# Run jobs ---------------------------------------------------------------------
plyr::l_ply(1:n_jobs, .fun = split_n_run, runs, phenotype, phenofile, pedigree, geno, dirlist)
